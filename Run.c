...
float M0 = -11.5f; //水平位置前馈
float rs03_torque=-11.5f,rs03_rad=-0.22f;
...
void rs03_PID_Init() {
	...
	rs03_vel_pid.Kp = 7.0f;
	rs03_vel_pid.Ki = 0.05f;
	rs03_vel_pid.Kd = 5.0f;
	rs03_vel_pid.limit = 20.0f;
	rs03_vel_pid.output_limit = 35.0f;
	
	
	rs03_pos_pid.Kp = 95.0f;
	rs03_pos_pid.Ki = 0.05f;
...
void Motor_Drive(void *param)
{
	...
	for(;;)
	{
		  ...
		  float M1 = M0 * cos(rs03.state.rad- 0.05);//初始水平的角度
...
//////			}
			...
			float target_omega = rs03_pos_pid.pid_out;
			PID_Control(curr_omega, target_omega, &rs03_vel_pid);			
